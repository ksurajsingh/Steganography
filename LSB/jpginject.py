...
#! Python for injecting data inside a JPEG using LSB steganography
# Note: JPEG is lossy, so LSB may be less reliable than PNG
...
# Save the stego image (JPEG with high quality to preserve LSB)
img.save(output_image, 'JPEG', quality=95)
print(f"Successfully embedded {len(secret_data)} bytes into {output_image}")
print("Note: JPEG is lossy - use PNG for better reliability")
